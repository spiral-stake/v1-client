...
const Borrow = ({
  ...
  const [userCollateralBalance, setUserCollateralBalance] =
    useState<BigNumber>();
  const [userCollateralAllowance, setUserCollateralAllowance] =
    useState<BigNumber>();
  ...
  const updateUserCollateralAllowance = async () => {
    ...
    if (borrowToken.address == positionManager.stblUSD.address) {
      allowance = await new ERC20(collateralToken).allowance(
        address,
  ...
  const handleLtvSlider = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const _adjustedStblUSD = BigNumber(amountCollateralInUsd).multipliedBy(
      BigNumber(ltv).div(100)
    );
    setAmountStblUSD(String(_adjustedStblUSD));
  };

  async function handleApprove() {
    if (!positionManager || !collateralToken || !borrowSwapper || !borrowToken)
      return;

    if (borrowToken.address == positionManager.stblUSD.address) {
      await new ERC20(collateralToken).approve(
        positionManager.address,
        amountCollateral
      );
    } else {
  ...
  async function handleBorrow() {
    ...
    if (borrowToken.address == positionManager.stblUSD.address) {
      ...
      await borrowSwapper.openPosition(
        collateralToken,
        borrowToken,
        amountCollateral,
        amountStblUSD
                    ...
                    <div className="rounded-sm border border-[#142435]">
                      <div className="grid grid-cols-[auto_1fr_auto] gap-x-3 sm:gap-x-8 md:grid-cols-[auto_auto_auto_1fr_auto]">
                        <Action
                          ...
                          completed={userCollateralAllowance?.gte(
